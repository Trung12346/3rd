...
public class AdminHoanTienController {
    ...
    public String chiTiet(@PathVariable Integer id, Model model, RedirectAttributes ra) {
        ...
        // Neu hoa don chua co tyLeHoan (mot so ly do: don cu, checkout, NV tao truc tiep...)
        // -> tinh lai theo rule moi de trang chi tiet LUON hien thi % hoan chinh xac.
        // Neu trang thai don la Da nhan phong / Da tra phong -> tyLe = 0 (khong hoan),
        // nhung van set 0 (khong de null) de template khong hien thi "-".
        if (dp != null) {
     ...
     * Áp dụng khi tỷ lệ hoàn = 0% (khách hủy muộn theo rule) hoặc NV/Admin chọn không hoàn.
